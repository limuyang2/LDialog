...
# LDialog
一个基于Goolegle推荐的DialogFragment封装的的库，根据自身业务提取封装，本库全部均使用```kotlin```语言编写，```java```亦可调用，能满足大部分的项目需求。本项目准守最大化的自由程度。  
...
> 源码说明：  
> 如果你还没上手koltin，建议学习使用。使用的环境版本如下：  
> * kotlin 1.2.51
...
## 获取
本库分为必须倒入的```LDialog```和非必须的```CustomLDialog```。  
...
## 简单使用
### CustonLDialog 使用方式
CustonLDialog是继承BaseLDialog扩写的类，目前里面包含3种自定义样式：
* IOSMsgDialog
* MaterialMsgDialog
...
### LDialog 使用方式 (主要使用方式)
LDialog也是继承BaseLDialog的类，用对外提供使用 layoutRes 自定义布局。  
示例如下：
```kotlin
//kotlin
...
| 通用方法                 |                                                              |
|setBackgroundDrawableRes|窗口背景资源文件id|
| setTag                   | DialogFragment的标签                                         |
| setDismissListener       | 关闭监听                                                     |
...
## 高级使用
如果以上仍然无法满足你们的需求，那可以直接继承```BaseLDialog```类。  
基本写法如下：
```kotlin
